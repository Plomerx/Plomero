let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Alguien todavía está haciendo \ nText Treasure Throne \ n en este chat ... espera a que termine'
  else conn.hartatahta[m.chat] = true
  m.reply('_Creando ..._ \n*Espere aproximadamente 1 minuto*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, '.png', '*Ghost-bot*\nEspero que te guste', m)
  } finally {
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let layers = [
      ...
      textArgs('', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2`, w, h) + format + '[mid]',
