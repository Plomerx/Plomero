let handler = async (m, { conn, args, participants }) => {
  ...
  let text = `
• *Parte superior de la tabla de clasificación de XP ${len}* •
Tu: *${usersExp.indexOf(m.sender) + 1}* dari *${usersExp.length}*

${sortedExp.slice(0, len).map(({ jid, exp }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${exp} Exp*`).join`\n`}

• *Límite superior de la tabla de clasificación ${len}* •
Tu: *${usersLim.indexOf(m.sender) + 1}* dari *${usersLim.length}*
...
• *Nivel superior de la tabla de clasificación ${len}* •
Tu: *${usersLevel.indexOf(m.sender) + 1}* dari *${usersLevel.length}*
